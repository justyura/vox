// Standalone one-shot whisper.cpp demo: transcribe a single 16kHz mono WAV
// file and print the result. This is the throwaway proving ground for the
// cgo wiring before the logic is lifted into internal/processor.
//
// Usage:
//
//	go run . [audio.wav]
//	go run . -model /path/to/ggml-tiny.bin jfk.wav
package main
...
func main() {
	modelPath := flag.String("model", "/Users/yura/personal/whisper.cpp/models/tiny.bin", "path to a ggml whisper model")
	flag.Parse()
